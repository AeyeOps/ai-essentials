# AEO Ghostty launcher — zsh rc blocks. The installer inserts each marker-fenced
# block separately: the GUARD near the top of ~/.zshrc (above the p10k
# instant-prompt block, because it `exec`s and would otherwise flash the prompt),
# and the INTEGRATION block at the very end. @FZF_PATH@ is replaced with the
# resolved fzf>=0.45 absolute path at install time.
...
if [[ -o interactive && -z "${TMUX:-}" && -n "${GHOSTTY_RESOURCES_DIR:-}" \
