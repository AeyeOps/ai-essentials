...
export class TerminalMapper implements vscode.Disposable {
  ...
  async focusSession(sessionId: string, log?: vscode.OutputChannel): Promise<void> {
    log?.appendLine(`focusSession: looking up cached terminal for ${sessionId}`);
    const cached = this.sessionTerminals.get(sessionId);
    if (cached) {
      log?.appendLine(`focusSession: found cached terminal "${cached.name}", calling show()`);
      cached.show();
      return;
    }

    log?.appendLine(`focusSession: no cached terminal, matching on-the-fly`);
    const session = this.discovery.getSession(sessionId);
    if (!session) {
      log?.appendLine(`focusSession: session not found in discovery`);
      return;
    }

    log?.appendLine(`focusSession: session pid=${session.pid}, walking ancestors`);
    const ancestors = getAncestorPids(session.pid);
    log?.appendLine(`focusSession: ancestors=[${[...ancestors].join(',')}]`);

    const termCount = vscode.window.terminals.length;
    log?.appendLine(`focusSession: checking ${termCount} terminals`);

    for (const t of vscode.window.terminals) {
      const pid = await t.processId;
      log?.appendLine(`focusSession: terminal "${t.name}" pid=${pid}`);
      if (pid !== undefined && (ancestors.has(pid) || session.pid === pid)) {
        log?.appendLine(`focusSession: MATCH! calling show()`);
    ...
    log?.appendLine(`focusSession: no terminal matched`);
  }
